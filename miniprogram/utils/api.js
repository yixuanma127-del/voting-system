...
function normalizeImages(items) {
  return (items || []).map(item => ({
    ...
    image_data: item.image_data
      ? ((item.image_data.startsWith('data:') || item.image_data.startsWith('http')) ? item.image_data : 'data:image/jpeg;base64,' + item.image_data)
      : absoluteImageUrl(item.image_url)
  }))
}
function getEvents() {
...
function getWorks(eventId) {
  ...
  return request(url).then(normalizeImages)
}
...
function getResults(eventId) {
  ...
  return request(url).then(data => normalizeImages(data.results || data))
}
